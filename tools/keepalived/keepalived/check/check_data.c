...
{
	zone *zone = data;
	FREE_PTR(zone->local_addr_gname);
	FREE(zone);
}
static void
dump_zone(void *data)
{
	...
	log_message(LOG_INFO, " zone addr = %s", inet_ntop(AF_INET, &zone->addr, buf, sizeof(buf)));
	log_message(LOG_INFO, " netmask = %s", inet_ntop(AF_INET, &zone->netmask, buf, sizeof(buf)));
	if (zone->local_addr_gname)
		log_message(LOG_INFO, " LOCAL_ADDR GROUP = %s", zone->local_addr_gname);
}
...
void
alloc_zone(char *addr, char *netmask)
{
	zone *new;

	new = (zone *) MALLOC(sizeof (zone));

	inet_stozone(addr, netmask, &new->addr, &new->netmask);
	
	new->local_addr_gname = NULL;

	list_add(check_data->zone, new);
}
...
alloc_check_data(void)
{
	...
	new->zone = alloc_list(free_zone, dump_zone);
	return new;
}
...
dump_check_data(check_conf_data *check_data)
{
	...
	if (!LIST_ISEMPTY(check_data->vs)) {
		...
		if (!LIST_ISEMPTY(check_data->zone))
			dump_list(check_data->zone);
