...
ipvs_start(void)
{
	...
	zrule = (ipvs_zone_t *) MALLOC(sizeof(ipvs_zone_t));
...
ipvs_talk(int cmd)
{
	...
	switch (cmd) {
		...
		case IP_VS_SO_SET_ADDLADDR:
			result = ipvs_add_laddr(zrule, laddr_rule);
			break;
		case IP_VS_SO_SET_DELLADDR:
			result = ipvs_del_laddr(zrule, laddr_rule);
			break;
		case IP_VS_SO_SET_ADDZONE:
			result = ipvs_add_zone(zrule);
			break;
		case IP_VS_SO_SET_DELZONE:
			result = ipvs_del_zone(zrule);
			break;
...
static int
ipvs_laddr_cmd(int cmd, zone * zone)
{
	local_addr_group *laddr_group = ipvs_get_laddr_group_by_name(zone->local_addr_gname, 
							check_data->laddr_group);
	if (!laddr_group) {
		log_message(LOG_ERR, "No address in group %s", zone->local_addr_gname);
		return IPVS_ERROR;
	}

	memset(zrule, 0, sizeof(ipvs_zone_t));
	zrule->__addr_v4= zone->addr;
	zrule->addr.ip = zone->addr;
	zrule->netmask= zone->netmask;
	zrule->af = AF_INET;
...
static int
ipvs_zone_cmd(int cmd, zone * zone)
{
...
int 
zone_cmd(int cmd, zone *zone)
{
	/* Set/Remove local address */
	if (cmd == IP_VS_SO_SET_ADDLADDR || cmd == IP_VS_SO_SET_DELLADDR)	
		return ipvs_laddr_cmd(cmd, zone);

	if (cmd == IP_VS_SO_SET_ADDZONE || cmd == IP_VS_SO_SET_DELZONE)	
		return ipvs_zone_cmd(cmd, zone);
	return IPVS_ERROR;
}
...
int
ipvs_laddr_remove_entry(zone *zone, local_addr_entry *laddr_entry)
{
	memset(zrule, 0, sizeof(ipvs_zone_t));
