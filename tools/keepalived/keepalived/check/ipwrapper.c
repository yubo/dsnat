...
/* Remove zone rule */
static int
clear_zone(zone *zone)
{
	
	/* Processing real server queue */
	// get zone -> laddr_list
	
	if (!zone_cmd(LVS_CMD_DEL_ZONE, zone))
		return 0;
...
init_zone(zone * zone)
{
	...
	/*Set local ip address in "FNAT" mode of IPVS */
	if(!ISALIVE(zone)){
		iret = zone_cmd(LVS_CMD_ADD_ZONE, zone);
		if (!iret)
			return 0;
		else
			SET_ALIVE(zone);
	...
	iret = zone_cmd(LVS_CMD_ADD_LADDR, zone);
	if (!iret)
		return 0; 
...
svr_checker_up(int alive,  checker_id_t cid, real_server *rs)
{
	...
	if(!alive && !ISALIVE(rs) && rs->set &&
				(rs->inhibit != 2) && (LIST_SIZE(l) > 0)) {
		element next;
...
static int
clear_diff_laddr_entry(list old, list new, zone *old_zone)
{
	element e;
	...
	for (e = LIST_HEAD(old); e; ELEMENT_NEXT(e)) {
		...
		if (!laddr_entry_exist(laddr_entry, new)) {
			log_message(LOG_INFO, "zone [%s-%d] in local address group %s no longer exist\n" 
					    , inet_sockaddrtos(&laddr_entry->addr)
					    , laddr_entry->range
					    , old_zone->local_addr_gname);

			if (!ipvs_laddr_remove_entry(old_zone, laddr_entry))
				return 0;
		}
	}
...
static int
clear_diff_laddr(zone * old_zone)
{
	local_addr_group *old;
	local_addr_group *new;

	/* Fetch local address group */
	old = ipvs_get_laddr_group_by_name(old_zone->local_addr_gname, 
							old_check_data->laddr_group);
	new = ipvs_get_laddr_group_by_name(old_zone->local_addr_gname, 
							check_data->laddr_group);

	if (!clear_diff_laddr_entry(old->addr_ip, new->addr_ip, old_zone))
		return 0;
	if (!clear_diff_laddr_entry(old->range, new->range, old_zone))
		return 0;
...
int 
clear_diff_zones(void)
{
	element e;
	list l = old_check_data->zone;
	zone *zone;
	char abuf[INET6_ADDRSTRLEN], nbuf[INET6_ADDRSTRLEN];

	/* If old config didn't own zone then nothing return */
	if (LIST_ISEMPTY(l))
		return 1;
	...
	for (e = LIST_HEAD(l); e; ELEMENT_NEXT(e)) {
		zone = ELEMENT_DATA(e);
		/*
		 * Try to find this zone into the new conf data
		 * reloaded.
		 */
		if (!zone_exist(zone)) {
			log_message(LOG_INFO, "Removing zone %s/%s"
					    , inet_ntop(AF_INET, &zone->addr, abuf, sizeof(abuf))
					    , inet_ntop(AF_INET, &zone->netmask, nbuf, sizeof(nbuf)));

			/* Clear VS entry */
			if (!clear_zone(zone))
				return 0;
		} else {
			/* If zone exist, perform local address diff */
			if (!clear_diff_laddr(zone))
				return 0;
		}		
	}
	return 1;
