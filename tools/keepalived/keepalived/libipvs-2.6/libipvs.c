...
typedef struct ipvs_zoneladdr_s {
	struct ip_vs_zone_kern		zone;
...
#define CHECK_COMPAT_ZONE(s, ret) CHECK_IPV4(s, ret)
...
static void ipvs_service_entry_2_user(const ipvs_service_entry_t *entry, ipvs_service_t *user)
{
...
#ifdef LIBIPVS_USE_NL
static int ipvs_nl_fill_zone_attr(struct nl_msg *msg, ipvs_zone_t *zone)
{
	struct nlattr *nl_zone;

	nl_zone = nla_nest_start(msg, IPVS_CMD_ATTR_ZONE);
	if (!nl_zone)
		return -1;
	
	NLA_PUT(msg, IPVS_ZONE_ATTR_ADDR , sizeof(zone->addr), &(zone->addr));
	NLA_PUT_U32(msg, IPVS_ZONE_ATTR_NETMASK, zone->netmask);
	
	nla_nest_end(msg, nl_zone);
	return 0;
...
static int ipvs_nl_fill_laddr_attr(struct nl_msg *msg, ipvs_laddr_t * laddr)
{
	...
	NLA_PUT(msg, IPVS_LADDR_ATTR_ADDR , sizeof(laddr->__addr_v4), &(laddr->__addr_v4));
...
int ipvs_add_zone(ipvs_zone_t *zone)
{
	ipvs_func = ipvs_add_zone;

#ifdef LIBIPVS_USE_NL
	if (try_nl) {
		struct nl_msg *msg = ipvs_nl_message(IPVS_CMD_NEW_ZONE , 0);
		if (!msg) return -1;
		if (ipvs_nl_fill_zone_attr(msg, zone))
			goto nla_put_failure;
		return ipvs_nl_send_message(msg, ipvs_nl_noop_cb, NULL);
...
nla_put_failure:
	...
	CHECK_COMPAT_ZONE(zone, -1);

	return setsockopt(sockfd, IPPROTO_IP, IP_VS_SO_SET_ADDZONE, (char *)zone,
			sizeof(struct ip_vs_zone_kern));
}


int ipvs_del_zone(ipvs_zone_t *zone)
{
	ipvs_func = ipvs_del_zone;

#ifdef LIBIPVS_USE_NL
	if (try_nl) {
		struct nl_msg *msg = ipvs_nl_message(IPVS_CMD_DEL_ZONE , 0);
		if (!msg) return -1;
		if (ipvs_nl_fill_zone_attr(msg, zone))
			goto nla_put_failure;
		return ipvs_nl_send_message(msg, ipvs_nl_noop_cb, NULL);
...
nla_put_failure:
	...
	return setsockopt(sockfd, IPPROTO_IP, IP_VS_SO_SET_DELZONE, (char *)zone,
			sizeof(struct ip_vs_zone_kern));
}
...
int ipvs_add_laddr(ipvs_zone_t *zone, ipvs_laddr_t * laddr)
{
	ipvs_zoneladdr_t zoneladdr;
...
nla_put_failure:
	...
	memcpy(&zoneladdr.zone, zone, sizeof(zoneladdr.zone));
	memcpy(&zoneladdr.laddr, laddr, sizeof(zoneladdr.laddr));

	return setsockopt(sockfd, IPPROTO_IP, IP_VS_SO_SET_ADDLADDR,
			(char *)&zoneladdr, sizeof(zoneladdr));
}

int ipvs_del_laddr(ipvs_zone_t *zone, ipvs_laddr_t * laddr)
{
	ipvs_zoneladdr_t zoneladdr;
...
static int ipvs_zones_parse_cb(struct nl_msg *msg, void *arg)
{
	struct nlmsghdr *nlh = nlmsg_hdr(msg);
	struct nlattr *attrs[IPVS_CMD_ATTR_MAX + 1];
	struct nlattr *zone_attrs[IPVS_ZONE_ATTR_MAX + 1];
	struct ip_vs_get_zones **getp = (struct ip_vs_get_zones **)arg;
	struct ip_vs_get_zones *get = (struct ip_vs_get_zones *)*getp;
	int i = get->num_zones;

	if (genlmsg_parse(nlh, 0, attrs, IPVS_CMD_ATTR_MAX, ipvs_cmd_policy) != 0)
		return -1;

	//check zone cmd suport
	if (!attrs[IPVS_CMD_ATTR_ZONE])
		return -1;

	if (nla_parse_nested(zone_attrs, IPVS_ZONE_ATTR_MAX, attrs[IPVS_CMD_ATTR_ZONE], ipvs_zone_policy))
		return -1;

	memset(&(get->entrytable[i]), 0, sizeof(get->entrytable[i]));


	if (!(zone_attrs[IPVS_ZONE_ATTR_ADDR] &&
				zone_attrs[IPVS_ZONE_ATTR_NETMASK])){
		return -1;
	}


	memcpy(&(get->entrytable[i].addr), nla_data(zone_attrs[IPVS_ZONE_ATTR_ADDR]),
			sizeof(get->entrytable[i].addr));
	get->entrytable[i].netmask = nla_get_u32(zone_attrs[IPVS_ZONE_ATTR_NETMASK]);
	...
	get->num_zones = i;
	get = realloc(get, sizeof(*get) + sizeof(ipvs_zone_entry_t) * (get->num_zones + 1));
	*getp = get;
	return 0;
}
...
struct ip_vs_get_zones *ipvs_get_zones(void)
{
	struct ip_vs_get_zones *get;
	struct ip_vs_get_zones_kern *getk;
	socklen_t len;
	int i;

	ipvs_func = ipvs_get_zones;

#ifdef LIBIPVS_USE_NL
	if (try_nl) {
		struct nl_msg *msg;
		len = sizeof(*get) +
			sizeof(ipvs_zone_entry_t);
		if (!(get = malloc(len)))
			return NULL;
		get->num_zones = 0;

		msg = ipvs_nl_message(IPVS_CMD_GET_ZONE, NLM_F_DUMP);
		if (msg && (ipvs_nl_send_message(msg, ipvs_zones_parse_cb, &get) == 0))
			return get;
	...
	len = sizeof(*get) +
		sizeof(ipvs_zone_entry_t) * ipvs_info.num_zones;
	if (!(get = malloc(len)))
		return NULL;
	len = sizeof(*getk) +
		sizeof(struct ip_vs_zone_entry_kern) * ipvs_info.num_zones;
	if (!(getk = malloc(len)))
		return NULL;

	getk->num_zones = ipvs_info.num_zones;
	if (getsockopt(sockfd, IPPROTO_IP,
				IP_VS_SO_GET_ZONES, getk, &len) < 0) {
		free(get);
		free(getk);
		return NULL;
	}
	memcpy(get, getk, sizeof(struct ip_vs_get_zones));
	for (i = 0; i < getk->num_zones; i++) {
		memcpy(&get->entrytable[i], &getk->entrytable[i],
				sizeof(struct ip_vs_zone_entry_kern));
		get->entrytable[i].af = AF_INET;
		get->entrytable[i].addr.ip = get->entrytable[i].__addr_v4;
	}
	free(getk);
...
ipvs_zone_entry_t * ipvs_get_zone(__be32 addr, __be32 netmask)
{
	ipvs_zone_entry_t *zone;
	socklen_t len;

	ipvs_func = ipvs_get_zone;

	len = sizeof(*zone);
	if (!(zone = malloc(len)))
		return NULL;
	memset((void *)zone, 0x00, len);
	...
	if (try_nl) {
		...
		ipvs_zone_t tzone;
		tzone.af = AF_INET;
		tzone.__addr_v4 = addr;
		tzone.addr.ip = addr;
		tzone.netmask = netmask;

		if (!(get = malloc(sizeof(*get) + sizeof(ipvs_zone_entry_t))))
			goto ipvs_get_zone_err2;

		get->num_zones = 0;

		msg = ipvs_nl_message(IPVS_CMD_GET_ZONE, 0);
		if (!msg) goto ipvs_get_zone_err;
		if (ipvs_nl_fill_zone_attr(msg, &tzone))
			goto nla_put_failure;
		if (ipvs_nl_send_message(msg, ipvs_zones_parse_cb, &get))
			goto ipvs_get_zone_err;

		memcpy(zone, &(get->entrytable[0]), sizeof(*zone));
		free(get);
		return zone;
...
ipvs_get_zone_err2:
	...
	if (getsockopt(sockfd, IPPROTO_IP, IP_VS_SO_GET_ZONE,
				(char *)zone, &len)) {
		free(zone);
		return NULL;
	}
...
struct ip_vs_get_laddrs *ipvs_get_laddrs(ipvs_zone_entry_t *zone)
{
	...
	len = sizeof(*l) + sizeof(ipvs_laddr_entry_t) * zone->num_laddrs;
	if (!(l = malloc(len)))
		return NULL;
	...
	if (try_nl) {
		struct nl_msg *                 msg;
		struct nlattr *                 nl_zone;
		if (zone->num_laddrs == 0)
			l = realloc(l,sizeof(*l) + sizeof(ipvs_laddr_entry_t));
		l->addr.ip = zone->addr.ip;
		l->netmask = zone->netmask;
		l->num_laddrs = zone->num_laddrs;
		...
		NLA_PUT(msg, IPVS_ZONE_ATTR_ADDR, sizeof(zone->addr),
				&zone->addr);
		NLA_PUT(msg, IPVS_ZONE_ATTR_NETMASK,  sizeof(zone->netmask),
				&zone->netmask);
...
ipvs_nl_laddr_failure:
	...
	len = sizeof(*lk) + sizeof(struct ip_vs_laddr_entry_kern) * zone->num_laddrs;
	if (!(lk = malloc(len)))
		return NULL;

	lk->addr = zone->addr.ip;
	lk->netmask = zone->netmask;
	lk->num_laddrs = zone->num_laddrs;
...
const char *ipvs_strerror(int err)
{
	...
	struct table_struct {
		...
		{ ipvs_get_laddrs, ESRCH, "Zone not defined" },
		{ ipvs_add_zone, EEXIST, "Zone already exists" },
		{ ipvs_del_zone, ESRCH, "No such zone" },
		{ ipvs_del_zone, ENOENT, "No such zone" },
		{ ipvs_get_zone, ESRCH, "No such zone" },
